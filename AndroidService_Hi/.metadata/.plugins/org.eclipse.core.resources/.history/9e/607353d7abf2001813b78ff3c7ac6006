...
public class UserDao {
	...
	public User findByUsername(String username, String password) {
		...
		if(!list.isEmpty()){
			...
			user.setImgurl((String)list.get(0).get("imgurl"));
		}
		return user;
	}
	...
	public int insertUser(User user) {
		String sql="insert into user(username,password,email,phone,imgurl) value(?,?,?,?,?)";
		Object[] params={
				user.getUsername(),user.getPassword(),user.getEmail(),user.getPhone(),user.getImgurl()
		};
